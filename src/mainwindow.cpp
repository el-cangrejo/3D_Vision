...
void MainWindow::connectUI() {
  ...
  connect(ui->action_ExportOFF, SIGNAL(triggered()), this,
	  SLOT(onActionExportOFF()));
  ...
  connect(ui->action_Load_Query_Database, SIGNAL(triggered()), this,
	  SLOT(onActionLoadQueryDatabase()));
  ...
  connect(ui->action_Load_Query_Classes, SIGNAL(triggered()), this,
	  SLOT(onActionLoadQueryClasses()));
  connect(ui->action_PR_Curves, SIGNAL(triggered()), this,
	  SLOT(onActionPRCurves()));
  ...
  connect(ui->ShowDBQuery, SIGNAL(clicked(bool)), this,
	  SLOT(setShowDBQuery(bool)));

  // Connect Buttons
  connect(ui->GridFilter, SIGNAL(clicked()), this, SLOT(gridFilter()));
  connect(ui->SegmentatImgButton, SIGNAL(clicked()), this,
	  SLOT(onSegmentImg()));
  connect(ui->GenerateMeshButton, SIGNAL(clicked()), this,
	  SLOT(onGenerateMesh()));
  connect(ui->DrawButton, SIGNAL(clicked()), this, SLOT(onDraw()));
  connect(ui->ClearAllButton, SIGNAL(clicked()), this, SLOT(onClearAll()));
  connect(ui->ZoomImgWidgetButton, SIGNAL(clicked()), this,
	  SLOT(onZoomImgWidget()));
  connect(ui->ComputeDescriptorsButton, SIGNAL(clicked()), this,
	  SLOT(onComputeDescriptors()));
  connect(ui->RetrievalButton, SIGNAL(clicked()), this, SLOT(onRetrieve()));
  connect(ui->DeleteDB, SIGNAL(clicked()), this, SLOT(onDeleteDB()));
  connect(ui->PartialViewButton, SIGNAL(clicked()), this,
	  SLOT(onPartialView()));
  connect(ui->BilateralFilterButton, SIGNAL(clicked()), this,
	  SLOT(onBilateralFilter()));
  connect(ui->MultilateralFilterButton, SIGNAL(clicked()), this,
	  SLOT(onMultilateralFilter()));
  connect(ui->InvertNormalsButton, SIGNAL(clicked()), this,
	  SLOT(onInvertNormals()));

  // Segmenation
  connect(ui->median_kernelSpinBox, SIGNAL(valueChanged(int)), this,
	  SLOT(setMedianKernel(int)));
  connect(ui->normal_radiusSpinBox, SIGNAL(valueChanged(int)), this,
	  SLOT(setNormalsRadius(int)));
  connect(ui->edge_radiusSpinBox, SIGNAL(valueChanged(int)), this,
	  SLOT(setEdgeRadius(int)));
  connect(ui->KernelRadiusSpinBox, SIGNAL(valueChanged(int)), this,
	  SLOT(setKernelRadius(int)));

  // Database
  connect(ui->GridSizeDB, SIGNAL(valueChanged(double)), this,
	  SLOT(setGridSizeDB(double)));

  // Mesh operations
  connect(ui->S_dSpinBox, SIGNAL(valueChanged(double)), this,
	  SLOT(setSdBF(double)));
  connect(ui->S_nSpinBox, SIGNAL(valueChanged(double)), this,
	  SLOT(setSnBF(double)));

  ui->GridSize->setDecimals(4);

  connect(ui->ChoosePointSpinBox, SIGNAL(valueChanged(int)), this,
	  SLOT(setChosenVertex(int)));
}
...
void MainWindow::onComputeDescriptors() {
  if (ui->widget->primary_meshes.empty()) {
    std::cout << "\e[1;31mNo mesh loaded!\nPlease load mesh first and then try "
		 "to compute desriptors!\e[0m\n";
    return;
  }
...
void progressBar(int progress, int end) {
	int x = progress / (float) end * 100;
	std::cout << "\r\e[1;31;43m";	
	for (int i = 0; i <= x; ++i) 
		std::cout << " ";
	std::cout << "\e[0m";
...
void MainWindow::onRetrieve() {
  ...
  for (int i = 0; i < ui->dbwidget->processed_database_meshes.size(); ++i) {
    float dist = ui->widget->filtered_mesh.distanceTo(
	ui->dbwidget->processed_database_meshes[i]);
		progressBar(i, ui->dbwidget->processed_database_meshes.size());
  }
	std::cout << "\n";
  end = clock();
  ...
  std::sort(ui->dbwidget->processed_database_meshes.begin(),
	    ui->dbwidget->processed_database_meshes.end(),
	    [](const Mesh &a, const Mesh &b) -> bool {
	      return a.overall_distance < b.overall_distance;
	    });

  std::sort(ui->dbwidget->database_meshes.begin(),
	    ui->dbwidget->database_meshes.end(),
	    [](const Mesh &a, const Mesh &b) -> bool {
	      return a.overall_distance < b.overall_distance;
	    });

  for (const auto &m : ui->dbwidget->database_meshes)
    std::cout << "\e[1;32mDistance :" << m.overall_distance << "\e[0m\n";
...
void MainWindow::onPartialView() {
  takePartialView(ui->dbwidget->database_meshes, 0.40);
}
...
void MainWindow::OpenMesh(QString fileName) {
  if (!fileName.isEmpty()) {
    ...
    if (m.load(fileName.toStdString()) != 0) {
      m.preprocess();
      m.computeNormals();
      ui->NumOfPointsLabel->setText(QString::fromStdString(
	  "Num of Points : " + std::to_string(m.vertices.size())));
      ...
      // << "\n";
      // std::cout << "Primary mesh  triangle std : " <<
      // ui->widget->primary_meshes[0].trianglesAreaStd() << "\n";
...
void MainWindow::onActionExportOFF() {
	QString fileName = QFileDialog::getSaveFileName(this);
	ui->widget->primary_meshes[0].exportOff(fileName.toStdString());
}
...
void MainWindow::onActionOpenKinect() {
  if (ui->Kinect_Widget->kinect_initialized) {
    ...
    ui->Kinect_Widget->kinect_initialized =
	!ui->Kinect_Widget->kinect_initialized;
  ...
  if (ui->Kinect_Widget->serial == "") {
    ui->Kinect_Widget->serial =
	ui->Kinect_Widget->freenect2.getDefaultDeviceSerialNumber();
  }
  /// [discovery]

  if (ui->Kinect_Widget->pipeline) {
    /// [open]
    ui->Kinect_Widget->dev = ui->Kinect_Widget->freenect2.openDevice(
	ui->Kinect_Widget->serial, ui->Kinect_Widget->pipeline);
    /// [open]
  } else {
    ui->Kinect_Widget->dev =
	ui->Kinect_Widget->freenect2.openDevice(ui->Kinect_Widget->serial);
  }
  ...
  ui->Kinect_Widget->dev->setIrAndDepthFrameListener(
      &ui->Kinect_Widget->listener);
  /// [listeners]

  /// [start]
  if (ui->Kinect_Widget->enable_rgb && ui->Kinect_Widget->enable_depth) {
    if (!ui->Kinect_Widget->dev->start()) return;
  } else {
    if (!ui->Kinect_Widget->dev->startStreams(ui->Kinect_Widget->enable_rgb,
					      ui->Kinect_Widget->enable_depth))
      return;
  }

  std::cout << "device serial: " << ui->Kinect_Widget->dev->getSerialNumber()
	    << std::endl;
  std::cout << "device firmware: "
	    << ui->Kinect_Widget->dev->getFirmwareVersion() << std::endl;
  /// [start]
...
void MainWindow::onActionLoadDatabase() {
  ...
  if (!directory_name.isEmpty()) {
    std::cout << "Loading for display database : "
...
void MainWindow::onActionLoadQueryDatabase() {
  QString directory_name = QFileDialog::getExistingDirectory(this);
  if (!directory_name.isEmpty()) {
    std::cout << "Loading for display database : "
	      << directory_name.toStdString() << "\n";
    loadDB(directory_name.toStdString(), ui->dbwidget->query_database_meshes);
    ui->dbwidget->updateGL();
  }
}
...
void MainWindow::onActionPreprocessDatabase() {
  if (ui->dbwidget->database_meshes.empty()) {
    std::cout << "\e[1;31mNo Database meshes loaded!\nPlease use action Load "
		 "Database and try again!\e[0m\n";
    return;
  }

  preprocessDB(ui->dbwidget->database_meshes,
	       ui->dbwidget->processed_database_meshes);

  if (!ui->dbwidget->query_database_meshes.empty()) {
		std::cout << "Processing Query Database Meshes!\n";
    preprocessDB(ui->dbwidget->query_database_meshes, ui->dbwidget->processed_query_database_meshes);
  }
}
...
void MainWindow::onActionLoadClasses() {
  ...
  readObjectClasses(file_name.at(0).toStdString(),
		    ui->dbwidget->database_meshes);
}

void MainWindow::onActionLoadQueryClasses() {
  QStringList file_name = QFileDialog::getOpenFileNames(this);
  if (file_name.isEmpty()) {
    std::cout << "Empty file!\nDatabase classes not loaded!\n";
    return;
  ...
  std::cout << "Loading query database classes!\n";
  readObjectClasses(file_name.at(0).toStdString(),
		    ui->dbwidget->query_database_meshes);
}

void MainWindow::onActionPRCurves() {
  precisionRecall(ui->dbwidget->processed_database_meshes,
		  ui->dbwidget->processed_query_database_meshes);
}
...
void MainWindow::setShowDatabase(bool show) {
  ui->dbwidget->setShowDatabase(show);
}
...
void MainWindow::setShowDBQuery(bool show) { ui->dbwidget->setShowQuery(show); }
...
void MainWindow::setMultiMesh(bool show) { ui->widget->setMultiMesh(show); }
...
void MainWindow::gridFilter() {
  // ui->widget->filtered_mesh =
  // ui->widget->primary_meshes[0].partialView(0.33);
...
void MainWindow::onSegmentImg() {
  ...
  ImgSegmenter segm(ui->SnapShot_Widget->getImg(), median_kernel,
		    normals_radius, edge_radius, kernel_radius);
...
void MainWindow::onGenerateMesh() {
  ...
  if (ui->Colored_Widget->mask_bool) {
    ...
    segm.estimateNormals();
    segm.writetoMesh(ui->widget->primary_meshes[0], 1,
		     ui->Colored_Widget->getImg(), ui->Colored_Widget->mask);
    ui->Colored_Widget->mask_bool = false;
    ui->widget->primary_meshes[0].preprocess();
    ui->ChoosePointSpinBox->setMaximum(
	ui->widget->primary_meshes[0].vertices.size());
    ui->widget->updateGL();
  } else {
    // segm.writetoMesh(ui->widget->primary_mesh, 1);
...
void MainWindow::onBilateralFilter() {
  if (ui->widget->filtered_mesh.empty())
    ui->widget->filtered_mesh =
	ui->widget->primary_meshes[0].bilateralFilter(sd_bf, sn_bf);
  else
    ui->widget->filtered_mesh =
	ui->widget->filtered_mesh.bilateralFilter(sd_bf, sn_bf);
  ui->widget->updateGL();
}
...
void MainWindow::onMultilateralFilter() {
  if (ui->widget->filtered_mesh.empty())
    ui->widget->filtered_mesh =
	ui->widget->primary_meshes[0].multilateralFilter(0.03);
  else
    ui->widget->filtered_mesh =
	ui->widget->filtered_mesh.multilateralFilter(0.03);
  ui->widget->updateGL();
  ui->widget->filtered_mesh.computeNormals(1);
}

void MainWindow::onInvertNormals() {
	Mesh &m = ui->widget->primary_meshes[0];
...
void MainWindow::setMedianKernel(int x) { median_kernel = x; }

void MainWindow::setNormalsRadius(int x) { normals_radius = x; }

void MainWindow::setEdgeRadius(int x) { edge_radius = x; }

void MainWindow::setKernelRadius(int x) { kernel_radius = x; }

void MainWindow::setGridSizeDB(double grid_size) {
  for (auto &m : ui->dbwidget->database_meshes) m.setGridSize(grid_size);
  for (auto &m : ui->dbwidget->query_database_meshes) m.setGridSize(grid_size);
}

void MainWindow::setSdBF(double x) { sd_bf = x; }

void MainWindow::setSnBF(double x) { sn_bf = x; }
