...
Mesh Mesh::gridFilter(void) {
  ...
  float radius = sqrt(pow(grid_size / 2., 2) +
                      pow(grid_size / 2., 2) +
                      pow(grid_size / 2., 2));
  ...
  for (int grid_x = 0; grid_x < dim_x; ++grid_x) {
    for (int grid_y = 0; grid_y < dim_y; ++grid_y) {
      for (int grid_z = 0; grid_z < dim_z; ++grid_z) {
        ...
        if (!fpfhist.empty()) hist.resize(fpfhist[0].size());
...
Mesh Mesh::statoutFilter(void) {
    ...
    std::cout << "Mesh points after stat out filter : " << filtered_mesh.vertices.size() << "\n";
...
float local_distance(const Mesh &query_mesh, const Mesh &target_mesh){
  ...
  for (size_t idx_q = 0; idx_q < query_mesh.fpfhist.size(); idx_q += 2) {
    ...
    ++count;
    for (int i = 0; i < k; ++i) {
      //std::cout << "Distance to add = " << (1. / k) * (1 - (float)i / k) * min_dists[i] << "\n";
...
void preprocess_database(std::string db_path) {
    ...
    for (boost::filesystem::directory_iterator dir(p);
        ...
        if(target_file.substr(target_file.size() - 3, 3) != "obj") continue;
...
void load_database(std::string db_path,
                   std::vector<Mesh>& db_descr,
    ...
    for (boost::filesystem::directory_iterator dir(p);
        ...
        if(target_file.substr(target_file.size() - 3, 3) != "dtr") continue;

        Mesh target_mesh;
        target_mesh.fpfhist = std::move(load_descriptors(target_file));
        std::string target_obj = target_file.substr(0, target_file.size() - 3) + "obj";
